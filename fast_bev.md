...
### neck_fuse 尺寸统一
根据 multi_scale_id，选择某一层（比如 P2 或 P5），把其他层 resize 到同样大小，然后 concat 在通道维度，最后卷积压缩：
```
...
**情况二：multi_scale_id=[3]**

👉 表示选择 P5 作为融合基准。

基准层：P5 [6, 64, 29, 50]

把 P2、P3、P4 都 resize 到 (29, 50)

P2 [6, 64, 232, 400] → [6, 64, 29, 50]

P3 [6, 64, 116, 200] → [6, 64, 29, 50]

P4 [6, 64, 58, 100] → [6, 64, 29, 50]

Concat：
[6, 64*4, 29, 50] = [6, 256, 29, 50]

再经过一个 3×3 Conv（neck_fuse_3）降到 64：
输出： [6, 64, 29, 50]
